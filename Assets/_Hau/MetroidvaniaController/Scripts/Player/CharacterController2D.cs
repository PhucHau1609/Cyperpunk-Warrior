...
public class CharacterController2D : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            ...
            {
                OnLandEvent.Invoke();
                if (!m_IsWall && !isDashing && !isDashingY)
    ...
    public void Move(float move, bool jump, bool dashX, bool dashY)
    {
        float jumpDirection = gravityController != null && gravityController.IsGravityInverted() ? -1f : 1f;
        if (canMove)
        {
            if (dashX && canDash && !isWallSliding)
            {
            ...
            // ⚠️ MỚI: Logic dash Y
            if (dashY && canDash && !isWallSliding)
            {
                StartCoroutine(DashYCooldown());
            }
            ...
            {
                m_Rigidbody2D.linearVelocity = new Vector2(0, m_DashForceY * jumpDirection);
            }
            ...
            else if (m_IsWall && !m_Grounded)
            {
                ...
                // ⚠️ MỚI: Dash Y từ wall
                else if (dashY && canDash)
                {
                    isWallSliding = false;
                    animator.SetBool("IsWallSliding", false);
                    oldWallSlidding = false;
    ...
    */
    ...
    // ⚠️ MỚI: COROUTINE DASH Y
    IEnumerator DashYCooldown()
    {
        animator.SetBool("IsDashingY", true);
        isDashingY = true;
        canDash = false;
        yield return new WaitForSeconds(0.1f);
        isDashingY = false;
